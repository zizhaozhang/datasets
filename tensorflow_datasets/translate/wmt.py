...
class SubDataset(object):
  ...
  def __init__(self, name, target, sources, url, path, manual_dl_files=None):
    ...
    self.manual_dl_files = manual_dl_files
...
_TRAIN_SUBSETS = [
    ...
    SubDataset(
        ...
        path=("training-parallel-nc-v12/news-commentary-v12.{src}-en.{src}",
              "training-parallel-nc-v12/news-commentary-v12.{src}-en.en")),
    SubDataset(
    ...
    SubDataset(
        ...
        path=("rapid2016/rapid2016.{0}-{1}.{src}",
              "rapid2016/rapid2016.{0}-{1}.en")),
    SubDataset(
    ...
    SubDataset(
        ...
        path=("rapid2019.de-en/rapid2019.de-en.de",
              "rapid2019.de-en/rapid2019.de-en.en")),
    SubDataset(
        name="setimes_2",
        target="en",
        sources={"tr"},
    ...
    SubDataset(
        ...
        target="en",
        sources={"ru", "zh"},
        url="https://cms.unov.org/UNCorpus/",
        manual_dl_files=["UNv1.0.en-ru.tar.gz"],
        # TODO(adarob): get path
        path=""),
    SubDataset(
        name="uncorpus_v1_zh",
        target="en",
        sources={"zh"},
        url="https://cms.unov.org/UNCorpus/",
        manual_dl_files=["UNv1.0.en-zh.tar.gz"],
        # TODO(adarob): get path
        path=""),
    SubDataset(
    ...
    SubDataset(
        ...
        path=("1mcorpus/corpus.en_ru.1m.ru", "1mcorpus/corpus.en_ru.1m.en")),
    ...
    SubDataset(  # pylint:disable=g-complex-comprehension
        ...
        path=("{0}/{0}_ch.txt".format(ss), "{0}/{0}_en.txt".format(ss)))
...
class WmtTranslate(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    def _check_manual_files(ds):
      ...
      manual_paths = []
      for fname in ds.manual_dl_files:
        ...
        if not tf.io.gfile.exists(manual_path):
          raise AssertionError(
              ...
              "from {1} and place them in {2}: {3}".format(
                  ...
                  ", ".join(ds.manual_dl_files)))
        manual_paths.append(manual_path)
      if len(ds.manual_dl_files) > 1:
        return manual_paths
      return manual_paths[0]
...
def _parse_parallel_sentences(f1, f2):
  ...
  def _parse_text(path):
    ...
    if split_path[-1] == "txt":
      # CWMT
      lang = split_path[0].split("_")[-1]
      lang = "zh" if lang == "ch" else lang
    else:
      lang = split_path[-1]
  ...
  l1_sentences, l1 = parse_file(f1)
  l2_sentences, l2 = parse_file(f2)

  assert len(l1_sentences) == len(l2_sentences), (
      "Sizes do not match: %d vs %d for %s vs %s." % (
          len(l1_sentences), len(l2_sentences), f1, f2))
...
def _parse_tmx(path):
  ...
  def _get_tuv_lang(tuv):
    for k, v in tuv.items():
      if k.endswith("}lang"):
        return v
    assert False, "Language not found in `tuv` attributes."
  ...
  for tu in etree.iterfind("body/tu"):
    yield {
        _get_tuv_lang(tuv): _get_tuv_seg(tuv) for tuv in tu.iterfind("tuv")
    }
...
def _parse_czeng(*paths, **kwargs):
  ...
  if filter_path:
    ...
    logging.info(
        "Loaded %d bad blocks to filter from CzEng17.", len(bad_blocks))
