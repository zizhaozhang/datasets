...
class Coco2014caption(tfds.core.GeneratorBasedBuilder):
  """MS Coco dataset."""
  ...
  def _generate_examples(
    ...
    if has_annotation:
      ...
      caption_filename = "captions_{}.json"
    else:
    ...
    if caption_filename is None:
      caption_path = None
    else:
      caption_path = os.path.join(
          annotation_dir,
          "annotations",
          caption_filename.format(split_type),
      )
    
    coco_annotation = CocoAnnotation(instance_path, caption_path)
    ...
    for image_info in sorted(images, key=lambda x: x["id"]):
      if has_annotation:
        ...
        captions = coco_annotation.get_captions(img_id=image_info["id"])
      else:
...
class CocoAnnotation(object):
  ...
  def _load_caption_annotations(caption_annotations_file):
    # Modified from https://github.com/tensorflow/tpu/blob/master/tools/datasets/create_coco_tf_record.py
    # and fixed the bug.
    ...
    for annotation in caption_annotations['annotations']:
      ...
      img_to_caption_annotation[image_id].append(annotation['caption'])
    ...
    if missing_annotation_count > 0:
      tf.logging.error('Require all images have caption annotation')
    return img_to_caption_annotation
    
  def __init__(self, annotation_path, caption_path=None):
    ...
    if "annotations" in data and caption_path is not None:  # Testing set don't has any annotations
      self._img_id2captions = CocoAnnotation._load_caption_annotations(caption_path)
  ...
  def get_captions(self, img_id):
    """Return all annotations associated with the image id string."""
    # Some images don't have any annotations. Return empty list instead.
    return self._img_id2captions.get(img_id, [])